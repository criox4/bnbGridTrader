...
| 🟡 | Fund mainnet + `GRID_MONITOR=1` | agent wallet funded (4 USDT + 0.0101 BNB) and a **full grid round trip executed on mainnet**: buy 2 USDT at level 4 → `cancel` sold it back → realised -0.0019991 USDT (= the 2×5bps fee, no price move to earn on), `completed_grids: 1`. A **`decide()`-driven sell** was then proven too, by re-shaping to 500–600/9 so the lot's sell target fell below spot (~2.3% rungs, no guard weakened). Monitor still `GRID_MONITOR=0` — no autonomous trading. Full suite: `python tools/e2e_test.py [--live]` |
